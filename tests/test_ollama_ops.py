...
import pytest
from lora_pipeline.ollama_client import call_qwen, analyse_with_backoff, TimeoutError
...
class TestCallQwen:
    ...
    def test_returns_parsed_json(self):
        ...
        with patch("lora_pipeline.ollama_client._ollama.chat",
                   return_value=self._resp(payload)):
            assert call_qwen("b64", cfg.MODEL)["folder"] == "solo"

    def test_strips_markdown_fences(self):
        payload = "```json\n" + json.dumps({"folder": "group"}) + "\n```"
        with patch("lora_pipeline.ollama_client._ollama.chat",
                   return_value=self._resp(payload)):
            assert call_qwen("b64", cfg.MODEL)["folder"] == "group"

    def test_raises_on_bad_json(self):
        with patch("lora_pipeline.ollama_client._ollama.chat",
                   return_value=self._resp("NOT JSON")):
            with pytest.raises(json.JSONDecodeError):
                call_qwen("b64", cfg.MODEL)

    def test_raises_timeout_when_hung(self, monkeypatch):
        monkeypatch.setattr(cfg, "CALL_TIMEOUT", 0.1)
        def hang(*a, **kw): import time; time.sleep(10)
...
GOOD = {
    "folder": "solo", "caption": "test", "quality_score": 4,
    "has_face": True, "face_visibility": "clear", "shot_type": "close-up",
    "subject_angle": "frontal", "nsfw": False, "lighting": "natural",
    "occlusion": [], "background": "simple", "compression": "clean",
    "filters_detected": False, "rejection_reason": None, "tags": [],
}
...
class TestAnalyseWithBackoff:
    ...
    def test_retries_on_json_error_then_succeeds(self):
        calls = [json.JSONDecodeError("bad", "", 0),
                 json.JSONDecodeError("bad", "", 0), GOOD]
    ...
    def test_returns_error_after_max_retries(self, monkeypatch):
        ...
        with patch("lora_pipeline.ollama_client.call_qwen",
    ...
    def test_attempts_ollama_restart_on_connection_error(self, monkeypatch):
        ...
        with patch("lora_pipeline.ollama_client.call_qwen", side_effect=calls):
            with patch("lora_pipeline.ollama_client.ollama_running", return_value=False):
                with patch("lora_pipeline.ollama_client.restart_ollama",
                           return_value=True) as mock_restart:
                    with patch("time.sleep"):
