...
def process_single(src: Path, model: str, dry_run: bool = False) -> dict:
    """Process one image end-to-end. Returns the metadata record. Raises on failure."""
    img = load_image(src)
    res = resolution_meta(img)
    ph = get_phash(img)
    b64 = image_to_b64(img)
    dup = check_duplicate(ph, src.stem)

    analysis = analyse_with_backoff(b64, src.stem, model)
    ...
    if not dry_run:
        ...
        meta_path = config.OUTPUT_DIR / "_metadata" / f"{src.stem}.json"
        atomic_write(meta_path, json.dumps(record, indent=2, ensure_ascii=False))

        dest = config.OUTPUT_DIR / folder / f"{src.stem}{src.suffix.lower()}"
        ...
        try:
            ...
            if "No space left" in str(e):
                raise OSError(f"Disk full while copying {src.name}") from e
            raise
        atomic_write(config.OUTPUT_DIR / folder / f"{src.stem}.txt", txt)
...
def run(args, model: str, shutdown_event):
    ...
    for i, src in enumerate(images, 1):
        ...
        t0 = time.time()
        try:
            record = process_single(src, model, args.dry_run)
        except Exception as e:
            log.error(f"[{i:4}/{total}] FAIL  {src.name}: {e}")
            errors += 1
            append_failed(stem)
            cp["errors"] = errors
            save_checkpoint(cp)
        ...
        nsfw = "NSFW" if record.get("nsfw") else "    "
        dup_tag = " DUP" if record.get("duplicate_of") else "    "
